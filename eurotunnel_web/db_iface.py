...
from eurotunnel_datamodel.DataModel import HumanConfirmations, SpringImageLocation, SpringLocation, TrainPass
...
"""
get_train_passes_by_install_sql: Final[str] = (
    "SELECT * FROM get_train_passes('{before}'::timestamp,{n_passes}::int,{install}::int);"
)
...
def get_n_train_passes_before_by_system(n: int, before_time: datetime, installation: int) -> list[dict]:
    ...
    with get_session() as db_session:
        ...
        res = db_session.execute(
            text(get_train_passes_by_install_sql.format(before=str(start_time), n_passes=n, install=installation))
        )
...
def car_row_to_web_dict(c) -> dict:
    ...
    return {
        ...
        "conf_code": confidence_interface.GetConfidenceInterface().confidence_level_car_and_spring(
            conf, c.all_confirmed
        ),
...
def get_train_pass_cars(train_pass_id: int, db_session=None) -> list[dict]:
    ...
    for i in range(len(result)):
        if (
            tail_icons[i] and result[i]["car_num"] >= len(result) / 2
        ):  # I expect this selection will need adjusting for PAX trains
...
def get_car_info_with_wheels(train_pass_id: int, train_order: int, db_session: Session | None = None):
    ...
    try:
        ...
        stmt = (
            select(SpringLocation, HumanConfirmations.present_not_absent)
            .join(HumanConfirmations, isouter=True)
            .where(
                SpringLocation.train_pass_id == train_pass_id,
                SpringLocation.train_axle_number >= axfirst,
                SpringLocation.train_axle_number < axlast,
            )
            ...
            spring_res.append(
                {
                    ...
                    "has_human_confirm": s[1] is not None,  # In theory this is redundant; you can just test for null
                }
            )
