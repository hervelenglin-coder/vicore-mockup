...
from eurotunnel_web.db_iface import get_train_pass_info_with_cars, get_car_info_with_wheels
from eurotunnel_web.user_management import create_users_if_none,authenticate_user
...
from eurotunnel_web.train_pass_endpoints import get_train_pass,load_train_passes,listpasses,get_train_passes
...
fallback_wheel_img_dir: Final[str] = '/mnt/c/Users/Graeme Yeo/Documents/ET Vision/eurotunnel_web/wheelimg'  #Used if the WHEEL_IMG_DIR environment variable is not set
...
app.add_url_rule('/put_confirmation_status/<spring_location>/<human_says>',view_func=put_confirmation_status,methods=['PUT'])


app.add_url_rule('/getTrainPass/<tpid>', methods=['POST'],view_func=get_train_pass)
#2 of the 3 parameters here are optional
app.add_url_rule('/loadTrainPasses/<last_shown>/<n_to_fetch>/<installation_id>', methods=['POST'],view_func=load_train_passes)
app.add_url_rule('/loadTrainPasses/<last_shown>/<n_to_fetch>',defaults={'installation_id': None}, methods=['POST'],view_func=load_train_passes)
app.add_url_rule('/loadTrainPasses/<last_shown>',defaults={'n_to_fetch': None,'installation_id': None}, methods=['POST'],view_func=load_train_passes)
...
def carview(train_pass_id, car_num):
    ...
    if not user_json:
        ...
        return render_template('carview.html', train_pass=tpic, current_car=car, wheel_img_root='/wheelimg'
                               ,username=user_model.name,
                               version = VERSION, 
...
if __name__ == '__main__':
    ...
    """
    create_users_if_none() #If there are no users in the DB, then create one before we start    
    app.run(host=os.environ.get('WEB_HOST'), port=int(os.environ.get('WEB_PORT', 5000)), debug=(os.environ.get('WEB_DEBUG', 'False') == True))
